...
from pythia.pyre.inventory import Uninit
...
class Facility(Trait, metaclass=Interface):
    ...
    def _getDefaultValue(self, instance):
        ...
        from pythia.pyre.inventory import Error
        locator = pythia.pyre.parsing.locators.error()
        ...
        if not self.default in [None, Uninit]:
            ...
            if isinstance(component, str):
                ...
                locator = pythia.pyre.parsing.locators.chain(loc, locator)
    ...
    def _set(self, instance, component, locator):
        if isinstance(component, str):
            ...
            import pythia.pyre.parsing.locators
            locator = pythia.pyre.parsing.locators.chain(source, locator)
    ...
    def _retrieveComponent(self, instance, componentName):
        ...
        if component is not None:
            ...
            if component is not None:
                ...
                if component:
                    locator = pythia.pyre.parsing.locators.simple('imported')
                else:
                    locator = pythia.pyre.parsing.locators.simple('not found')
                    return None, locator
    ...
    def _import(self, instance, name):
        ...
        if c == 1:
            ...
            raise Facility.ComponentNotFound(facility=self.name, component=name)
        module = path[0]
        factoryName = '.'.join(factoryPath)
        objName = module + ':' + factoryName

        try:
            from pythia.pyre.util import loadObject
            factory = loadObject(objName)
        except (ImportError, ValueError):
            raise Facility.ComponentNotFound(facility=self.name, component=name)
        except AttributeError:
            raise Facility.FactoryNotFound(facility=self.name, module=module, factory=factoryName)

        if not callable(factory):
            raise Facility.FactoryNotCallable(facility=self.name, module=module, factory=factoryName)
