...
from pythia.pyre.odb.fs.Curator import Curator as Base
...
class Curator(Base):
    ...
    def config(self, registry):
        ...
        entries = resource_listdir(pythia, "pythia")
        for entry in entries:
            entry_path = os.path.join("pythia", entry)
            if resource_isdir(pythia, entry_path):
                vault = entry_path + '/__vault__.odb'
                if resource_exists(pythia, vault):
                    builtin = self.createDepository(resource_filename(pythia, entry_path))
    ...
    def _registerCodecs(self):
        ...
        pml = pythia.pyre.inventory.codecPML()
        cfg = pythia.pyre.inventory.codecConfig()
        pcs = pythia.pyre.inventory.codecConfigSheet()

        import pythia.pyre.odb
        odb = pythia.pyre.odb.odb()

        import pythia.pyre.templates
        tmpl = pythia.pyre.templates.codecTmpl()
